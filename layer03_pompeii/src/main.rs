...
use winit::{WindowBuilder, Window, dpi::LogicalSize, Event, WindowEvent, EventsLoop};
use std::sync::Arc;
...
impl Pompeii {
    ...
    fn create_instance() -> Arc<Instance> {
        if ENABLE_VALIDATION_LAYERS && !Self::check_validation_layer_support() {
            println!("validation is not available!")
        }
        ...
        let appInfo = ApplicationInfo {
            application_name: Some("Pompeii".into()),
            application_version: Some(Version { major: 1, minor: 0, patch: 0 }),
            engine_name: Some("Custome".into()),
            engine_version: Some(Version { major: 0, minor: 0, patch: 0 }),
        };

        let required_extensions = Self::get_required_extensions();
        if ENABLE_VALIDATION_LAYERS && Self::check_validation_layer_support() {
            Instance::new(Some(&appInfo), &required_extensions, VALIDATION_LAYERS.iter().cloned())
                .expect("failed")
        } else {
    ...
    fn pick_physical_device(instance: &Arc<Instance>, surface: &Arc<Surface<Window>>) -> usize {
        PhysicalDevice::enumerate(&instance)
            ...
            .expect("do you have some GPU, fren?")
    }
    ...
    fn create_logical_device(instance: &Arc<Instance>,
                             surface: &Arc<Surface<Window>>,
                             physical_device_index: usize
